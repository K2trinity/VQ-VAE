...
def get_model(args):
    model = VQVAE(in_channels=args.in_channels, out_channels=args.out_channels, features=args.features, embedding_num=args.embedding_num, embedding_dim=args.embedding_dim, norm_type=args.norm_type)
    return model
...
if __name__ == '__main__':
    ...
    model = VQVAE(1, 1, [32,64,128], embedding_num=128, embedding_dim=128, norm_type=None)
    model.to(device)
