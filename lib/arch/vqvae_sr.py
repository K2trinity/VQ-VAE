...
class VQVAE_Generator(nn.Module):
    def __init__(self, in_channels:int=1, out_channels:int=1, features:list=[64,128,256], embedding_num:int=128, embedding_dim:int=256, norm_type=None):
        ...
        self.sr_decoder = Decoder(in_channels=embedding_dim, out_channels=out_channels, features=features[::-1], norm_type=norm_type, dim=3)
    ...
    def forward(self, z):
        ...
        z_sr = self.sr_decoder(z_q.detach())
        return z_e, z_q, z_rec, z_sr
...
class VQVAE_SR(nn.Module):
    def __init__(self, in_channels:int=1, out_channels:int=1, features:list=[64,128,256], embedding_num:int=128, embedding_dim:int=256, norm_type=None,
                 full_mip:bool=True, args=None):
        super(VQVAE_SR, self).__init__()
        self.full_mip = full_mip
        self.args = args

        self.G = VQVAE_Generator(in_channels=in_channels, out_channels=out_channels, features=features, embedding_num=embedding_num, embedding_dim=embedding_dim, norm_type=norm_type)
        self.D_MIP = CubeDiscriminator(in_channels=out_channels, features=features, norm_type=norm_type, dim=2)
        self.D_Rec = CubeDiscriminator(in_channels=out_channels, features=features, norm_type=norm_type, dim=3)
    
    def load_pretrain(self, ckpt_path, gpu):
        device = torch.device(f'cuda:{gpu}')
        ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
        self.G.vqvae.load_state_dict({k.replace('module.',''):v for k,v in ckpt['model'].items()})
    ...
    def feed_data(self, real_A_ori:torch.Tensor):
        ...
        for img, mip in zip(_real_A, _iso_mip):
            min_v, max_v, median_v = img.min(), img.max(), img.median()
            img = norm_fn(self.args.data_norm_type)(img)
            real_A.append(img)

            mip = torch.clip(mip, min_v, max_v)
            mip = norm_fn(self.args.data_norm_type)(mip, min_value=min_v, max_value=max_v, median=median_v)
            iso_mip.append(mip)
    ...
    def forward(self, z):
        if self.full_mip:
            z, ref_iso_mip = self.feed_data(z)
        z_e, z_q, z_rec, z_sr = self.G(z)
        if self.full_mip:
            return z, ref_iso_mip, z_e, z_q, z_rec, z_sr
        return z_e, z_q, z_rec, z_sr

def get_model(args):
    model = VQVAE_SR(in_channels=args.in_channels, out_channels=args.out_channels, features=args.features, 
...
if __name__ == '__main__':
    ...
    z, ref_iso_mip, z_e, z_q, z_rec, z_sr = model(z)